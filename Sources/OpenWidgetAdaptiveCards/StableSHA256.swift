struct StableSHA256 {
    ...
    static func hexDigest(of bytes: [UInt8]) -> String {
        let digits = Array("0123456789abcdef")
        var result = ""
        result.reserveCapacity(64)
        for byte in digest(of: bytes) {
    ...
    static func digest(of bytes: [UInt8]) -> [UInt8] {
        ...
        for offset in stride(from: 0, to: message.count, by: 64) {
            compress(block: message[offset..<(offset + 64)], state: &state)
        }
    ...
    private static func compress(
        ...
        for index in 16..<64 {
            ...
            let sigma0 = rotateRight(previous, by: 7)
                ^ rotateRight(previous, by: 18)
                ^ (previous >> 3)
            let sigma1 = rotateRight(previous2, by: 17)
